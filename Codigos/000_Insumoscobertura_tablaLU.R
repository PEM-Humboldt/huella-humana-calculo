...
corine_leyenda <- read_excel(file.path(dir_datos, "corine_mapbiomas.xlsx"))

# Vector de ejemplo
x <- c("[134] \"3.2.1.1.2.1. Herbazal denso inundable no arbolado\"")

# Limpiar el texto

# Extraer solo el nombre (todo después del primer espacio que sigue al código numérico)
x_limpio <- sub('.*\\d+\\.\\d+\\.\\d+\\.?\\d*\\.?\\d*\\.?\\d*\\.?\\s+', '', corine_leyenda$clc)  # soporta jerarquías largas
x_limpio <- gsub('"', '', x_limpio)  # elimina comillas si quedan

x_limpio[135] <- "Herbazal denso inundable no arbolado"

x_limpio

corine_leyenda$clc1 <- x_limpio
